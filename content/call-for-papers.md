---
title: "Call for Papers"
---
We invite original research and industry papers for the 1st edition of **HIDDEN LAYERS (Research Meets Practice): Enterprise Risk, AI Forensics, and Image Security**.

This workshop aims to bridge academic research with practical applications, with emphasis on enterprise image security and forensic AI.

## Workshop Topics

The workshop primarily focuses on enterprise risk in the context of AI-enabled and media-rich enterprise systems. Topics of interest include enterprise cybersecurity challenges, threat modeling and mitigation strategies, governance, compliance, and risk management, as well as the deployment and evaluation of forensic frameworks in enterprise environments. We welcome industry case studies that highlight real-world risks, vulnerabilities, and mitigation approaches, including human-centric threats such as social engineering, insider threats, and broader cybercrime activities that impact organizational security and resilience.

Building on this enterprise risk perspective, the workshop also covers technical advances in AI forensics and image security as key mechanisms for risk detection and investigation. Relevant topics include AI-based forensic methods, deep learning and anomaly detection for forensic analysis and enterprise risk detection, validation and robustness of forensic AI, and attacks against AI systems. We further invite contributions on image and digital media security, such as image steganography and steganalysis, detection of hidden or malicious content (e.g., stego-malware), deepfakes, fake news, AI-generated images, biometric security, and privacy-preserving AI techniques.

## Application Domains

We welcome empirical research papers, experience reports, and case studies from all application domains.

## Submission Types

Accepted submissions will follow the EDOC/CBI 2026 workshop proceedings format to publish in the joint proceedings volume (Springer LNBIP). We confirm that the workshop will adhere to Springer LNBIP requirements.

We accept the following submission types:

- Short Papers (4 pages) – concise studies or methodologies.
- Position Papers (4 pages) – emerging ideas, vision, or research positions.
- Full Papers (8 pages) – complete research or empirical studies.
- Industry Reports (4 pages) – practical case studies or challenges.

All papers submitted to the workshop will be peer-reviewed by three members of the Program Committee. Acceptance will be primarily based on originality, contribution to the area, potential to generate discussion, quality of the research, and completeness of the study. Only original papers not submitted elsewhere (journals or other conferences) will be considered.

## Workshop Venue

The workshop will be held in conjunction with **EDOC/CBI 2026** (September 15–18, 2026, University of Twente).

EDOC link: https://conf.researchr.org/home/cbi-edoc-2026

## Important Dates

- Paper submission: TBA
- Notification: TBA
- Camera-ready: TBA
